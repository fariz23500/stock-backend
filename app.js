...
app.get("/",function(req,res){
    ...
    data.forEach(d => {
       if(Number(d.date.substr(5,2))>Number(end.substr(5,2))&&Number(d.date.substr(8,2))>Number(end.substr(8,2)))return;
        if(d.symbol===symbol&&Number(d.date.substr(5,2))>=Number(start.substr(5,2))&&Number(d.date.substr(8,2))>=Number(start.substr(8,2)) && Number(d.date.substr(5,2))<=Number(end.substr(5,2))&&Number(d.date.substr(8,2))<=Number(end.substr(8,2)))
       { 
